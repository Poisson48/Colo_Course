...
if [[ -f "$REPO_DIR/deploy/releases/sync-recipes.sh" ]]; then
  install -m 0755 "$REPO_DIR/deploy/releases/sync-recipes.sh" /opt/colo-apps/sync-recipes.sh
  echo "Script recettes : /opt/colo-apps/sync-recipes.sh"
fi
